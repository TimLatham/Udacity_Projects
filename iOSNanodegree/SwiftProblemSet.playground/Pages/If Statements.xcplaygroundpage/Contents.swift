...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    ...
    if canFinishRun && canFinishBike {
        print("You are ready for the duathlon!")
    } else if canFinishBike || canFinishRun {
        canFinishRun ? print("You cannot finish the bike ride"):print("You cannot finish the run")
    } else {
        print("You cannot finish the bike or the run")
    }
}
